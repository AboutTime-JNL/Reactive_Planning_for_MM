...
#include "threading/WorkerOptions.hpp"
...
class WorkerManager {
   public:
    ...
    inline bool addWorker(const std::string& name, const double timestep,
        ...
        return addWorker(
            WorkerOptions(name, timestep,
                          std::bind(fp, obj, std::placeholders::_1), priority),
            autostart);
    }
    ...
    // the addWorker variant below is commented out because it can lead to
    // strange behaviour. E.g. when the user wants to move a worker from one
    // workerManager to
    ...
    /*!
     * Removes workers which are destructible (see Worker::isDestructible())
     * from the map (calling their destructors)
     */
