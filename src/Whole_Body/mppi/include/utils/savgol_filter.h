...
struct MovingExtendedWindow {
    MovingExtendedWindow(const int size, const int w) {
        window = w;
        // the first and last input need half horizon in the past and in the
        // future in order to be centered leaving with a total size equal to
        // weigths size (2 * window) + 1 + horizon/2
    ...
    void trim(const double t) {
        if (t < last_trim_t) {
            std::stringstream ss;
            ss << "Resetting the window back in the past. Can reset only to "
                  "larger "
                  "times than last "
                  "reset!!!"
               << "last reset=" << last_trim_t
               << ", trying to reset to t=" << t;
            throw std::runtime_error(ss.str());
        }
        last_trim_t = t;

        // search in the last inserted times the closest smaller than the
        // current
        unsigned int trim_idx = start_idx;
        ...
        unsigned int offset = trim_idx - window;
    ...
    std::vector<double> extract(const double t) {
        auto lower =
            std::lower_bound(tt.begin(), tt.end(),
                             t);  // index to the first element larger than t
        assert(lower != tt.end());
        unsigned int idx = std::distance(tt.begin(), lower);
    ...
    void set(const double u, const double t) {
        auto upper =
            std::upper_bound(tt.begin(), tt.end(),
                             t);  // index to the first element larger than t
        assert(upper != tt.end());
        unsigned int idx = std::distance(tt.begin(), upper) - 1;
        uu[idx] = u;
    }
...
class SavGolFilter {
   ...
   public:
    ...
    void apply(Eigen::Ref<Eigen::VectorXd, 0, Eigen::InnerStride<> > u,
               const double t);
