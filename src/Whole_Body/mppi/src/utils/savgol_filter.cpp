...
std::ostream& operator<<(std::ostream& os,
    ...
    os << "\nuu: [";
    for (unsigned int i = 0; i < w.uu.size(); i++) {
        os << w.uu[i] << " ";
    }
    os << "]\ntt: [";
    for (unsigned int i = 0; i < w.uu.size(); i++) {
...
SavGolFilter::SavGolFilter(const int steps, const int nu,
    ...
    if (window.size() == 1) {
        filters_.resize(nu, gram_sg::SavitzkyGolayFilter(
                                window[0], 0, poly_order[0], der_order));
...
void SavGolFilter::apply(Eigen::Ref<Eigen::VectorXd, 0, Eigen::InnerStride<>> u,
                         const double t) {
    for (long int i = 0; i < u.size(); i++) {
