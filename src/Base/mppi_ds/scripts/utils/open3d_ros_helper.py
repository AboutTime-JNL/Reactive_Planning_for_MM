...
def se3_to_transform(transform_nparray):
    ...
    r = Rotation.from_matrix(transform_nparray[:3, :3])
    quat = r.as_quat()
...
def rospc_to_numpy(rospc, remove_nans=False):
...
def points_to_rospc_wo_colors(points, frame_id=None, stamp=None):
...
def points_to_rospc(points, colors, frame_id=None, stamp=None):
    """ convert open3d point cloud to ros point cloud
    Args:
        o3dpc (open3d.geometry.PointCloud): open3d point cloud
        frame_id (string): frame id of ros point cloud header
...
def o3dpc_to_rospc(o3dpc, frame_id=None, stamp=None):
    ...
    if is_color:
        rgb_npy = np.asarray(copy.deepcopy(np.asarray(o3dpc.colors)))
        rgb_npy = np.floor(rgb_npy * 255)  # nx3 matrix
        rgb_npy = rgb_npy[:, 0] * BIT_MOVE_16 + rgb_npy[:, 1] * BIT_MOVE_8 + rgb_npy[:, 2]
        rgb_npy = rgb_npy.astype(np.uint32)
        data['rgb'] = rgb_npy
...
def p2p_icp_registration(source_cloud, target_cloud, n_points=100, threshold=0.02,
                         relative_fitness=1e-10, relative_rmse=1e-8, max_iteration=500,
    ...
    icp_result = open3d.registration.registration_icp(
        source=source_cloud, target=target_cloud, max_correspondence_distance=max_correspondence_distance,
        # unit in millimeter
        init=np.eye(4),
...
# def ppf_icp_registration(source_cloud, target_cloud, n_points=3000, n_iter=100, tolerance=0.001, num_levels=5,
#                          scale=0.001):
...
#     source_np_cloud = np.concatenate([np.asarray(source_cloud.points), np.asarray(source_cloud.normals)],
#                                      axis=1).astype(np.float32)
#     target_np_cloud = np.concatenate([np.asarray(target_cloud.points), np.asarray(target_cloud.normals)],
#                                      axis=1).astype(np.float32)
